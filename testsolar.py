...
# --- ANALYSIS DASHBOARD (RESTORED OLD LOOK) ---
class AnalysisWindow(ctk.CTkToplevel):
    def __init__(self, parent, df):
        ...
        y_true = valid['Actual_Output']
        y_phys = valid['Physical_Pred']
        y_stat = valid['Stat_Pred']
        ...
        # Header Section (The "Card" Look)
        header = ctk.CTkFrame(self, fg_color="transparent")
        header.pack(fill="x", padx=20, pady=10)
        ctk.CTkLabel(header, text="Prediction vs. Actual Analysis", font=("Bahnschrift", 22, "bold"), text_color="white").pack(pady=(0, 10))
        
        metrics_text = (f"PHYSICAL MODEL:\n  R² Score: {r2_p:.3f}\n  RMSE: {rmse_p:.3f} kWh\n\n"
                        f"AI MODEL (Scaled {parent.scaling_factor}x):\n  R² Score: {r2_s:.3f}\n  RMSE: {rmse_s:.3f} kWh")
        
        ctk.CTkLabel(header, text=metrics_text, font=("Consolas", 12), fg_color="#333333", corner_radius=8, padx=20, pady=15).pack()

        # Plotting Section (Side-by-Side)
        ...
        lims2 = [0, max(y_true.max(), y_stat.max())]
        ax2.plot(lims2, lims2, 'r--', alpha=0.7)
...
class SolarForecastApp(ctk.CTk):
    ...
    def slice_data_by_duration(self, data):
        ...
        if self.last_plot_type == "forecast":
            now = pd.Timestamp.now() - pd.Timedelta(hours=12) 
    ...
    def plot_graph(self, data, title, is_forecast=False, comparison=False):
        ...
        # RESTORE TOOLBAR
        toolbar = NavigationToolbar2Tk(canvas, ctk.CTkFrame(self.graph_frame))
        toolbar.update()
        toolbar.pack(fill="x")
