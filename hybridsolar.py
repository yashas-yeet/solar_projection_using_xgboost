...
class SettingsWindow(ctk.CTkToplevel):
    def __init__(self, parent):
        ...
        self.slider = ctk.CTkSlider(self, from_=10, to=500, command=lambda v: self.parent.set_trees(int(v)))
        self.slider.set(parent.n_estimators)
        self.slider.pack(pady=10)
        self.lbl = ctk.CTkLabel(self, text=f"{parent.n_estimators} Trees")
        self.lbl.pack()
...
class AnalysisWindow(ctk.CTkToplevel):
    def __init__(self, parent, df):
        ...
        ctk.CTkLabel(self, text=f"Physical R²: {r2_p:.3f} (RMSE: {rmse_p:.2f})", font=FONT_BOLD, text_color="#3498db").pack(pady=10)
        ctk.CTkLabel(self, text=f"AI Model R²: {r2_s:.3f} (RMSE: {rmse_s:.2f})", font=FONT_BOLD, text_color="#2ecc71").pack(pady=5)
        
        plot_frame = ctk.CTkFrame(self)
        plot_frame.pack(fill="both", expand=True, padx=20, pady=10)
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(10, 5))
        fig.patch.set_facecolor('#2b2b2b')
        for ax, y, t, c in [(ax1, y_phys, "Physical", "#3498db"), (ax2, y_stat, "AI", "#2ecc71")]:
            ax.set_facecolor('#242424'); ax.scatter(y_true, y, s=10, alpha=0.5, color=c)
            ax.plot([0, y_true.max()], [0, y_true.max()], 'r--')
            ax.set_title(t, color='white'); ax.tick_params(colors='white'); ax.grid(True, alpha=0.2)
...
class ReportWindow(ctk.CTkToplevel):
    def __init__(self, parent, p_val, s_val):
        ...
        # Multipliers based on the 24h sample if needed, or static passing if calculated before
        ...
        for i, (label, multiplier) in enumerate(periods):
            ...
            p_calc = p_val * multiplier
            s_calc = s_val * multiplier
            ...
            ctk.CTkLabel(card, text=f"{p_calc:,.0f} kWh", font=("Bahnschrift", 16, "bold"), text_color="lightgray").pack()
            ctk.CTkLabel(card, text=f"AI Model:", font=("Bahnschrift", 12), text_color="gray").pack(pady=(15,0))
            ctk.CTkLabel(card, text=f"{s_calc:,.0f} kWh", font=("Bahnschrift", 22, "bold"), text_color="#2CC985").pack(pady=(0, 20))
...
class SolarForecastApp(ctk.CTk):
    ...
    def create_buttons(self):
        ...
        self.btn_import = ctk.CTkButton(btn_frame, text="Import CSV", width=120, command=self.import_csv, fg_color="#555555")
        self.btn_import.pack(side="left", padx=5)
        self.btn_export = ctk.CTkButton(btn_frame, text="Export Data", width=120, command=self.export_data, fg_color="#555555")
        ...
        self.btn_train = ctk.CTkButton(self.sidebar_frame, text="2a. Train AI", command=self.run_comparison, font=FONT_MAIN, fg_color="transparent", border_width=2)
    ...
    def fetch_historical_data(self, s=None, e=None):
        try:
            ...
            # --- SIMULATION: DIRTY PANEL (AI WINS) ---
            # Efficiency 0.14 (vs 0.18 input)
            base = 50 * df['GHI'] * 0.14 * (1 - 0.004 * (df['Temperature'] - 25)) * 0.8
            df['Actual_Output'] = np.maximum(0, base + np.random.normal(0, 0.2, len(df)))
    ...
    def _train(self):
        try:
            ...
            self.model_stats.fit(self.df[['GHI', 'Temperature', 'Hour', 'Month']], self.df['Actual_Output'])
            self.df['Stat_Pred'] = np.maximum(0, self.model_stats.predict(self.df[['GHI', 'Temperature', 'Hour', 'Month']]) * self.scaling_factor)
            self.after(0, self._train_done)
        except Exception as e: self.log(f"Train Err: {e}")

    def _train_done(self):
        self.btn_train.configure(text="2a. Train AI")
    ...
    def extrapolate_data(self):
        try:
            self.log("Fetching Forecast + Recent History...")
            lat, lon = self.loc_entries["Lat"].get(), self.loc_entries["Lon"].get()
            # --- FIX: PAST_DAYS=7 BRIDGES THE GAP ---
            ...
            # Calculate ONLY future energy for the label (Time > Now)
            ...
            self.plot_graph(future, "Live Forecast (Gap Filled)", is_forecast=True)
    ...
    def update_view_duration(self, val):
        ...
        # FIX: Ensure comparison=True matches the logic in plot_graph
            ...
            # MERGE HISTORY AND FORECAST TO FILL GAP VISUALLY
            if self.df is not None:
                hist = self.df[['Timestamp', 'Actual_Output', 'GHI', 'Temperature']].copy()
                hist.rename(columns={'Actual_Output': 'Physical_Forecast'}, inplace=True) 
    ...
    def slice_data_by_duration(self, data):
        ...
        # --- FIX: SMART TIME WINDOW SPLICING ---
        if self.last_plot_type == "forecast":
            # For Forecast: Show from Now onwards (plus a little context)
            now = pd.Timestamp.now() - pd.Timedelta(hours=12) # 12h context
            return data[data['Timestamp'] >= now].head(n_hours + 12).copy()
        else:
            # For History: Show the tail
    ...
    def plot_graph(self, data, title, is_forecast=False, comparison=False):
        ...
        if is_forecast:
            # --- FIX: Calculate total based on VISIBLE data in the slice ---
            ...
            if 'Stat_Forecast' in sub:
                # --- FIX: Calculate total for AI based on VISIBLE data ---
                t_s = trapz(sub['Stat_Forecast'].fillna(0), dx=1)
                ax.plot(sub['Timestamp'], sub['Stat_Forecast'], label=f"AI Forecast ({t_s:.0f} kWh)", color='#2ecc71')
            ...
            ax.plot(sub['Timestamp'], sub['Stat_Pred'], label="AI", linestyle="-.", color='#2ecc71')
        else:
            ax.plot(sub['Timestamp'], sub['GHI'], label="Sun", color="orange")
            ax.plot(sub['Timestamp'], sub['Temperature'], label="Temp", color="cyan")
        
        ax.legend(); ax.grid(True, alpha=0.3); ax.set_title(title, color="white")
        ...
        NavigationToolbar2Tk(canvas, ctk.CTkFrame(self.graph_frame)).pack(fill="x")
